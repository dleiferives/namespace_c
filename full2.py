...
def stage5_refactor_method_calls(code, hierarchy):
    """
    Refactors method calls in the code:
    - Transforms 'a@add(10)' to 'MyType_add(&a, 10)'
    - Transforms 'MyType@add(b, 10)' to 'MyType_add(b, 10)' or 'MyType_add(&a, 10)'
    """
    # Find all method calls using @
    method_call_pattern = r"(\b[a-zA-Z_][a-zA-Z0-9_]*@(?:\w+))\s*\(([^)]*)\)"
    
    # Function to replace method calls
    def replace_method_call(match):
        ...
        if '@' in method_call:
            ...
            return full_call  # No replacement
        
        # Determine if obj_or_type is a variable or a type
        # Search in global variables and functions
        is_type = False
        for var in hierarchy["global"]:
            if var["name"] == obj_or_type:
                obj_type = var["type"]
                obj_pointer = '*' in var["type"]
                break
        else:
            # Check if obj_or_type is a type (e.g., MyType@add)
            # We'll assume it's a type if it's in the type_metadata
            # or if it starts with an uppercase letter (common convention)
            if obj_or_type in hierarchy["functions"]:
                # It's a function, not a type
                return full_call
            elif re.match(r'^[A-Z][a-zA-Z0-9_]*$', obj_or_type):
                is_type = True
                obj_type = obj_or_type
                obj_pointer = False
            else:
                # Assume it's a variable and search in functions' declarations
                found = False
                for func in hierarchy["functions"].values():
                    ...
                    if found:
                        break
                if not found:
                    # Variable not found, cannot replace
                    return full_call
        
        # Build the transformed function name
        transformed_function_name = f"{obj_type.replace('*','').strip()}_{method_name}"
        
        # Determine if we need to pass the reference
        if not is_type:
            # It's an instance method; pass reference if not a pointer
            if obj_pointer:
                transformed_args = f"{obj_or_type}, {args}" if args else f"{obj_or_type}"
            else:
                transformed_args = f"&{obj_or_type}, {args}" if args else f"&{obj_or_type}"
        else:
            # It's a static method; pass the type name if needed or as provided
            transformed_args = f"{args}" if args else ""
        
        # Handle multiple arguments and spacing
...
if __name__ == "__main__":
    ...
    # Stage 5: Refactor Method Calls
    refactored_code = stage5_refactor_method_calls(input_code, hierarchy)
